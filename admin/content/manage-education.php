<?php 
    ...
    if (isset($_POST["simpan"])) {
        ...
        $queryProfile = mysqli_query($config, "SELECT * FROM education ORDER BY id DESC");
        
        if (mysqli_num_rows($queryProfile) > 0) {
            $rowProfile = mysqli_fetch_assoc($queryProfile);
            $id = $rowProfile["id"];
            $update = mysqli_query($config, "UPDATE education SET 
            title='$title',
            year='$year',
            description='$description' WHERE id='$id'");
            header("location:?page=manage-education&ubah=berhasil");

        }else{
            $insertQ = mysqli_query($config, "INSERT INTO education (title, year, description) VALUES ('$title', '$year', '$description')");           
            if ($insertQ) {
